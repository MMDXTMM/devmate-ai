...
`business-map` 响应中的 `onboarding` 是面向陌生开发者的第一层导览，包含：

- 项目描述与源码证据共同限定的项目定位；
- 按业务重要性排序的核心流程及其 HTTP 入口；
- 代表性 Controller → Service → 数据访问链路；
- 从实现代码识别的异常、状态变化等失败信号；
- 状态枚举、数据库表和推荐阅读顺序。

该结果固定标记为 `STATIC_CODE_EVIDENCE_V2`。它不会把类名推断包装成 LLM 结论，也不声明已经识别所有状态转换；动态路由、反射、事件驱动和运行时行为仍需结合测试、日志或后续模型归纳确认。
