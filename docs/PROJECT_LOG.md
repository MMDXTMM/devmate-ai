...
- 定位到多个配置不同的 `@SpringBootTest` 上下文连接同一个固定名称 H2 内存库，非事务测试数据跨上下文残留，测试结果依赖执行顺序。
- `test` Profile 改为每个 Spring 测试上下文使用随机 UUID 数据库名；同一上下文继续缓存，事务测试仍正常回滚。
- 不通过重跑 CI 或放宽数据库断言掩盖污染；修复后同时执行默认顺序和 Surefire 随机顺序回归。
- 该问题说明本机测试全绿只代表特定环境和顺序通过，CI 的独立 JDK 与执行顺序能发现测试隔离缺陷。
...
- 新增最外层请求关联过滤器：只沿用 1-64 位安全 `X-Request-Id`，缺失、含空白/换行或超长值统一替换为 UUID，避免日志注入和无界字段。
- 最终 ID 写入请求属性、响应头和 MDC；完成日志只记录方法、路径、状态、结果和耗时，不记录查询参数、Token、请求体、源码或 Prompt。
- 下游直接异常时单独记录 `outcome=FAILED`，避免响应状态尚为 200 时误判成功；MDC 在 `finally` 清理，防止线程复用造成跨用户串号。
- 前端统一 API 客户端读取并再次校验响应 ID，业务错误会显示该 ID；网络不可达时不伪造后端 ID。
- 当前只实现模块化单体内的 HTTP 关联，不宣称具备 OpenTelemetry 分布式追踪；`requestId` 也不替代可跨请求恢复的 `taskId/attemptKey`。
- 全量验证通过后端 146 项、前端 50 项、Benchmark Node 48 项和 Vue 生产构建；本轮没有数据库迁移、外部模型调用或付费请求。
- 详细设计和面试问题见 `REQUEST_CORRELATION.md`。
