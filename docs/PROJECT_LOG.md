...
## 2026-08-16：完成 Spring AI 中文项目深度理解报告闭环

- 在静态业务地图之上新增账户模型驱动的中文深度报告，输出项目摘要、架构说明、核心业务流程、数据变化、推荐阅读顺序和未知风险；页面必须显式点击才会产生模型费用。
- 模型输入固定当前项目 revision、静态导览和有限代码证据；输出只能引用服务端提供的 Chunk ID，Java 校验白名单后回填真实文件、行号和代码块，拒绝伪造证据。
- 使用 Spring AI `ChatClient.responseEntity(...)` 接收结构化结果，实际 provider、model、Prompt 版本、Token 和耗时进入任务审计；报告 JSON 不重复保存源码正文。
- V24 新增 `project_understanding_report`。`attempt_key` 保证一次点击幂等，`running_key` 阻止同项目同版本并发生成；模型调用置于短事务之间，完成、失败和超时恢复都会释放运行键。
- API 与 Vue 支持生成报告和查看最近报告；连接未配置、版本变化、证据不足、模型失败和非法证据均返回可读状态，不暴露 Key、Prompt、SQL 或完整源码。
- 自动化覆盖结构化模型适配、成功、重复付费请求、伪造证据、权限拒绝、超时恢复、V1-V24 迁移和前端报告证据展示；全量通过后端 178 项、前端 68 项、Benchmark Node 48 项及 Vue 生产构建。真实 DeepSeek/通义/OpenAI 输出质量仍需用户使用自己的 Key 页面验收，当前不宣称模型准确率；本机 MySQL 当前未监听 3306/3307，因此 V24 真实 MySQL 迁移留给页面验收启动时完成。

## 2026-08-16：完成 Spring AI 与代码审查主链路整合

- 在 Spring Boot 3.5 基线上引入 Spring AI 1.1.8 BOM、OpenAI 兼容模型模块和 ChatClient，不升级到需要 Spring Boot 4 的 Spring AI 2.0。
- 新增账户级 `SpringAiChatClientFactory`，根据服务端白名单中的 Base URL、当前账户解密后的 Key 和所选模型创建短生命周期 `ChatModel/ChatClient`。
- DeepSeek、通义千问和 OpenAI 的连接测试不再手写 Chat Completions 请求与响应 DTO，统一经过 Spring AI；保留 5 秒连接、30 秒读取超时以及 401/403、429、网络异常和空响应的中文错误边界。
- 固定流水线审查使用 `ChatClient.responseEntity` 生成结构化 Finding；Agent 使用 `ChatModel` 传递 Spring AI 工具定义和多轮 Tool 消息，但仍由 Java 编排器受控执行工具并保存审计。
- AI 审查从当前登录账户解析并固定 provider、model、Base URL 和解密后的 Key，页面模型选择开始真正影响 FIXED/AGENT 两种审查任务；旧的环境级 DashScope Chat 适配器和重复 DTO 已删除。
- 远程 DashScope Embedding 已迁移到 Spring AI `EmbeddingModel`；本地哈希 Embedding 继续用于离线测试和降级。
- 测试验证了结构化输出、Tool Call、工具结果回传、Token/finish reason 元数据、429 脱敏错误和账户模型绑定，并确认 Key 不进入请求 JSON 或业务响应；审查执行期间切换账户模型会被拒绝，避免同一任务混用两个模型。
- 全量验证通过后端 173 项、前端 68 项、Benchmark Node 48 项测试，Vue 生产构建与 `git diff --check` 通过。
- 本闭环未新增数据库迁移。下一步基于当前账户模型生成带代码证据的中文项目理解报告；真实厂商调用效果仍需用户使用自己的 API Key 完成页面验收。

## 2026-08-15：完成多提供方模型连接中心

- 新增 DeepSeek、通义千问和 OpenAI 模型目录，DeepSeek 使用当前 `deepseek-v4-flash/pro`，不继续引入已退出的旧别名。
- Vue 新增“大模型连接”入口，可选择模型、填写 API Key、保存并切换提供方、查看配置状态和显式测试连接。
- 连接按登录用户隔离；V23 新增 `user_model_connection`，API Key 使用 AES-GCM 加密并绑定用户与提供方后持久化，页面与接口不回显明文，后端重启后仍可恢复。
- 模型地址由服务端白名单固定；尚未把模型接入项目深度报告。
- 自动化验证通过后端 171 项、前端 68 项测试和 Vue 生产构建；V23 已在本地 MySQL 26.7 成功迁移，真实厂商连接仍需用户提供对应 API Key 后在页面验收。

## 2026-08-15：完成 R2 第二小闭环——新人项目导览

- “浏览代码结构”默认入口升级为新人导览，不再要求用户先从 Controller 和文件列表自行拼接业务含义。
- 导览基于已解析源码生成项目定位、能力标签、核心业务流程、接口入口、代表性实现链路、数据操作、失败信号、状态枚举、数据库表和推荐阅读顺序。
- 核心流程按业务重要性和实现证据排序，健康检查与辅助查询不再占据主要阅读位置；普通 `State` 类只有识别到真实枚举值时才作为状态模型展示。
- 每条流程可以继续下钻到业务接口和真实代码；原业务地图与文件结构保持可用，没有新增数据库迁移，也没有调用或冒充 LLM 总结。
- 使用本地 MySQL 中已解析的 `devmate-ai` 验证：识别 14 个接口模块、24 个数据库表定义、2 个有枚举值的状态模型，并将项目管理、源码导入、向量索引、Diff、静态分析和 AI 审查排入核心理解路径。
- 全量回归通过后端 166 项、前端 67 项测试、Vue 生产构建与 `git diff --check`；本地页面能够正常进入登录入口，登录后的导览交互由前端组件测试覆盖。
- 根据真实“美食打卡系统”验收反馈，将 `Blog/Shop/Upload/Voucher` 等机械类名升级为“探店笔记与互动、商铺查询与维护、探店图片管理、优惠券管理”等用户业务，并按已识别接口生成登录、商铺浏览、内容互动、关注、优惠券和秒杀下单主线；原 Controller、接口路径和实现代码继续作为证据保留。

## 2026-08-15：发布 v0.2.0

- 将已完成并验证的一键 Java 代码审查、选项式需求澄清、源码解析稳定性修复和静态证据业务地图整理为 `v0.2.0`。
- 版本发布前通过后端 165 项、前端 67 项测试、Vue 生产构建与 `git diff --check`；未完成的真实模型深层总结和项目生成能力不计入本版本承诺。

## 2026-08-15：完成 R2 第一小闭环——静态证据驱动的项目业务地图

- 将“浏览代码结构”的默认入口从逐文件列表调整为业务地图，先展示项目业务模块、功能点和 HTTP 接口，再查看实现代码；原文件结构页签继续保留。
- 新增 `GET /api/projects/{projectId}/business-map` 和功能详情接口，基于 Controller、Spring Web 映射注解、已解析方法调用及数据访问引用生成模块与接口视图。
- 功能详情按 Controller → Service → 关联实现的顺序展示真实代码、文件、行号、调用流程和 Mapper/Repository 操作，不把类方法清单冒充业务理解结果。
- 当前分析模式固定标记为 `STATIC_CODE_EVIDENCE_V1`，并在页面展示动态路由、反射和业务语义仍可能缺失的边界；尚未声称完成 LLM 深层业务总结。
- 新增后端业务地图成功/非法入口测试和前端业务地图/文件结构兼容测试；完整回归通过后端 165 项、前端 67 项、Vue 生产构建与 `git diff --check`。使用本地 MySQL 已导入的 `devmate-ai` 验证得到 14 个业务模块、36 个 HTTP 接口，并能从项目创建接口追踪到 Service 实现及两处 Mapper 写入证据。

## 2026-08-07：完成 R1 一键 Java 代码审查编排闭环

- 新增 V22 `review_workflow_run`，固定一次审查对应的源码导入、Diff、静态分析、Embedding 和 Agent 审查任务，并保存当前阶段、失败原因和恢复动作。
- 新增一键审查接口与 Vue 主入口；Java 编排器按五阶段顺序执行，任何阶段失败即停止，最近一次运行可回读，普通用户不再需要手工点击内部步骤。
- `attempt_key` 保证同一次点击幂等返回，`running_key` 唯一约束阻止同一项目并发运行；超过 30 分钟的运行会标记失败并释放重试入口。
- 排查并修复 MyBatis-Plus `updateById` 跳过 `null` 导致运行键无法释放的问题；完成、失败和超时路径改为显式 `SET running_key = NULL`，并增加状态测试。
- Agent 成功链路使用真实测试 Git 仓库贯通 AST、Diff、PMD、本地 Embedding、受控 `searchCode` Tool 和 Fake 模型；同时覆盖源码失败脱敏、重复请求、并发冲突、非法 UUID、最近运行查询和跨用户权限拒绝。
- 全量验证通过：后端 163 项、前端 66 项、Benchmark Node 48 项，Vue 生产构建与 `git diff --check` 通过；H2 从空库执行 V1-V22，本地 MySQL 26.7 从 V21 成功迁移到 V22。
- 真实模型准确性和完整 A/B 仍未验收，不能声称 Agent 优于固定流水线。下一闭环按用户最新目标调整为“中文项目业务地图”：解析结果只作为证据层，最终输出业务模块、核心流程、状态变化、数据关系和推荐阅读顺序。

## 2026-08-07：求职版主线收敛为 RAG Java 代码审查 Agent

- 用户确认当前目标不是继续扩大双模板产品，而是把已有审查底座收敛成可以演示、理解并写入简历的小而完整项目。
- 当前唯一开发主线调整为 `导入 Java Git 项目 → Diff 定位 → 静态分析 → Hybrid RAG → 受控 Tool Calling Agent → 结构化 Finding → 反馈与评测`。
- `generation` 模块、V21 数据结构和 Vue 需求澄清工作台全部保留，不删除也不回滚；在真实需求分析、工程生成和编译测试恢复前，不把该能力写入简历。
- 后续按 R1 一键审查、R2 中文证据报告、R3 真实 canary/固定评测、R4 演示与面试理解四个闭环推进；每个闭环均由用户页面验收后再继续。
- 暂不新增微服务、MQ、Redis、外部向量数据库、自动改码或自动提交，避免为展示技术扩大项目范围。
- 本次只调整产品优先级和文档，不改变数据库结构、不调用模型，也不删除任何已有功能。

## 2026-08-07：完成选项式需求澄清闭环

- 将纯文本 `RequirementQuestion` 扩展为业务问题、工程规则和重要取舍三类，并支持单选、多选、自由文本、AI 建议、推荐理由及选项影响。
- `guided-requirement-v2` 会按工单、预约、库存、内容和通用管理场景生成不同角色与核心流程方案；这是确定性规则 Provider，不宣称为真实模型动态分析。
- 答案区分用户选择、主动采用推荐、AI 代选、自定义和历史文本；AI 代选由服务端固定为真实推荐项，并生成可回读的中文摘要。
- 服务端拒绝未知/重复选项、单选多值、推荐项不一致、结构化与旧文本混交等非法输入；方案版本、并发更新、确认锁定和用户隔离边界保持不变。
- Vue 工作台升级为问题卡片，展示通俗说明、推荐标识和代码影响，并提供“采用 AI 推荐方案”“我不确定，由 AI 决定”和自定义补充入口。
- 不新增数据库迁移：V21 的 JSON 字段继续保存契约；旧 v1 问题回读为自由文本，旧请求继续转为 `LEGACY_TEXT`。
- 全量验证通过：后端 159 项、前端 62 项、Benchmark Node 48 项，Vue 生产构建与 `git diff --check` 通过；H2 从空库成功执行 V1-V21。
- 本地 MySQL 26.7 验证 V21 已是最新版本；浏览器使用真实工单需求完成“创建 v1 → 采用推荐/AI 代选 → 生成 v2 → 确认锁定”，页面控制台无警告或错误。
- 本闭环不调用 LLM、不生成项目文件；下一闭环是独立需求分析模型 API，不能复用代码审查 Prompt。

## 2026-08-07：确认“AI 先给方案，用户做低门槛选择”的交互原则

- 用户不应被要求直接回答并发、事务、幂等和架构等专业问题；Agent 应先表达自己的需求理解和工程判断。
- 业务问题使用通俗语言；高影响问题提供 2～4 个互斥选项，并标出推荐项、原因和各选项影响。
- 每个专业问题提供“采用推荐方案”和“我不确定，由 Agent 按行业常见方案决定”，同时保留自由补充能力。
- 产品身份保持为 Java 代码 Agent：LLM 负责需求分析和工程取舍，Java 编排器与受控 Tool 负责版本、文件生成、编译测试和失败恢复。
- 现有仓库不重建。`generation` 模块只依赖 `common/user`，与 `knowledge/review` 没有直接耦合；原有解析、RAG 和审查模块继续服务“理解已有项目”和生成后的质量门禁。
- 后续生成开发冻结无关旧模块，优先只修改 `generation/agent/tool` 和对应前端，减少回归范围与 Token 消耗。

## 2026-08-07：完成需求澄清与确认的第一条可运行链路

- 新增独立 `generation` 模块和 Flyway V21；生成会话不复用导入仓库的 `project` 表，避免两种产品模板状态混杂。
- 用户可以提交一句话需求，系统保存原始输入并生成需求摘要、架构建议、显式假设和四类工程追问。
- 用户答案会生成不可覆盖的新方案版本；使用数据库条件更新和 `(session_id, version_no)` 唯一约束防止并发版本冲突。
- 只有最新版本且全部必答问题完成后才允许确认；确认后会话和方案均进入 `CONFIRMED`，禁止继续修改。
- 新增“生成新项目”前端入口和需求澄清工作台，支持描述、回答、更新方案和确认，且明确提示代码生成 Tool 尚未接入。
- 当前 `guided-requirement-v1` 是确定性规则草案，不宣称已经由 LLM 动态分析；下一步接入独立需求分析模型端口，并覆盖超时、限流、非法 JSON 和重试边界。
- 新增 7 个后端接口/权限测试和 4 个前端组件/API 测试，覆盖创建、范围拒绝、反复修改、必答校验、最新版本确认、确认后拒绝修改和用户隔离。
- 全量验证通过：后端 156 项、前端 60 项、Benchmark Node 48 项，Vue 生产构建成功；H2 已从空库成功执行 V1-V21，`git diff --check` 通过。
- 本地 MySQL 26.7 已从 V20 成功迁移到 V21，两张需求澄清表可用且原有数据未被覆盖。浏览器自动化因本地地址安全策略未执行，前端交互以 Vitest 组件闭环和生产构建作为本次验收证据。

## 2026-08-07：确立“双模板”产品与工程生成主线

### 用户目标纠正

- 产品不是以代码审查为中心，也不只是把源码符号和检索结果展示出来。
- 新项目生成模板以一句话作为低门槛入口，但不会一次性盲目生成；Agent 先输出需求与架构草案，对关键缺口反向提问，用户确认后再生成代码。
- 第一版只生成全新的 Spring Boot 模块化单体，不修改导入项目，也不生成 Spring Cloud。
- 用户已确认第一版聚焦管理与业务流程型后端，例如工单、预约、库存、内容管理和商城后台；游戏、实时音视频和复杂支付结算暂不作为生成承诺。
- 已有项目理解是独立模板，负责让陌生开发者快速理解项目，而不是工程生成的前置步骤。

### 架构决定

- LLM 负责需求理解、追问、架构取舍、任务规划和总结；Java 编排器负责状态、权限、预算、执行顺序和失败恢复。
- 工程思维放在版本化 Prompt、架构规则、工程规范和验收策略中；Tool 保持参数化和确定性，负责文件生成、项目检查、Maven 验证和质量报告。
- 代码审查调整为两个模板共享的质量门禁，不作为产品主入口。
- 当前源码导入、AST、配置/数据库解析、RAG、Tool Calling、静态分析和审查继续作为共享底座，不推倒重写。

### 后续顺序

1. 需求会话、反向提问、需求/架构版本和确认状态机。
2. 隔离工作区、Spring Boot 模板和受控文件生成 Tool。
3. Maven 编译测试、有限修复循环和生成质量报告。
4. 已有项目的中文深层报告和证据问答评测。
5. 生成已有项目修改能力、前端模板和 Spring Cloud 均延后。

详细设计见 `docs/PRODUCT_MODES_AND_GENERATION_AGENT.md`。生成能力尚未实现，README 和简历材料只能标记为规划。

本文件记录影响项目定位、架构、开发顺序或简历目标的重要变化。普通代码修改不需要逐条记录。

## 2026-08-07：产品主线回归“导入、理解、开发”

- 用户实际目标是导入陌生 Java 项目后快速理解并开始开发；原项目列表同时展示导入、Diff、静态分析、向量化、检索、AI 审查和评测，暴露了过多实现步骤，使用体验更像内部调试台。
- 将产品定位调整为“智能项目理解与开发助手”，固定主流程为 `导入项目 → 一键解析 → 理解工作台 → 搜索/阅读源码 → 开始开发`；代码审查保留为有简历价值的高级能力。
- 一键解析只编排源码导入和知识索引，不再隐式执行 Diff、静态分析或模型调用；任一步失败即停止并保留后端已有失败状态。
- 新增项目理解工作台，基于真实 `knowledge_document` 和 `code_reference` 汇总源码、包、结构项、配置、数据库迁移和已解析引用，并提供源码结构浏览、项目提问和推荐阅读顺序。
- 项目列表只保留“解析项目/打开理解工作台”和项目信息编辑；Diff、静态检查、AI 审查、评测、重建和删除统一收纳进高级分析。
- 检索界面默认只要求输入想理解的问题，Top-K、Token 预算、检索模式和 Diff 上下文进入折叠的高级设置；本次未增加外部向量数据库、后端接口或模型调用。
- 新增前端测试覆盖解析依赖顺序、失败停止、READY 项目进入工作台、解析证据统计、推荐问题传递和列表隐藏高级操作。
- 全量验证通过后端 146 项、前端 54 项、Benchmark Node 48 项和 Vue 生产构建；使用本地 MySQL 的 9 个项目完成桌面与 390px 移动浏览器验收，页面控制台无警告或错误。
