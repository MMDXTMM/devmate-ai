...
2026-08-06 自动化回归结果：37 项前端测试与生产构建成功；两种 AI 审查均需显式触发并绑定最近成功 Diff 的字符串 ID、40 位 revision 和每次新生成的 UUID v4 attemptKey，前置失败保持零创建，项目切换后的旧响应和重复触发不会覆盖当前审查状态。Agent 工具链可见，反馈不会重新消费模型额度，失败任务不会被展示为无风险。评测页只计算已保存任务，不会自动调用模型。真实模型和 MySQL 联调结果记录在对应阶段文档。
