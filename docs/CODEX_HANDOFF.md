...
| 本轮交付 | `codex/import-header-chunks`：精确 `FILE_HEADER/IMPORT` Chunk 与两侧 Diff 映射，待提交 PR |
| 数据库 | H2 已从空库迁移到 V18；隔离 MySQL 26.7 已完成 V17→V18，健康检查与历史数据回读通过 |
| 测试基线 | 后端 133 项、前端 46 项、Benchmark Node 48 项和 Vue 生产构建全部通过 |
| 精确暂停点 | 全新 H2 + 公开 Git 8 场景已从 `6 FULL / 2 PARTIAL` 提升到 `8 FULL / 0 PARTIAL`；MySQL 历史 Diff 保留旧证据，真实模型仍未调用 |
| 下一小任务 | 提交并合并文件头 Chunk PR；随后继续不付费的工程任务，真实 AI canary 仍需显式确认 DashScope 密钥和额度 |
...
| 效果评测 | 完成数据、计算、Vue 工作台、样本、Git 历史、真实 Diff/标准答案与受控执行器 | 新解析器 H2 实跑 8 PASS、8 FULL；MySQL 已落库 7 条 `DEFECT`、1 条 `CLEAN`，真实 A/B 待完成 |
...
2026-08-07 已新增精确 `FILE_HEADER` 包声明和逐条 `IMPORT` Chunk，TARGET 使用持久化 Chunk，BASE 使用 Git Blob 内存解析。全新 H2 V1-V18 对同一公开仓库再次完整运行得到 `8 PASS / 8 FULL / 0 PARTIAL / 0 SKIPPED` 且无警告，没有调用 Embedding 或模型。此前 MySQL 历史 Diff 继续保留旧解析结果，未为了数字重写历史证据；相同 revision 的幂等导入也不会破坏已有 Chunk/Vector ID。
