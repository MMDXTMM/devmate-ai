# Java 代码审查 Agent 闭环与问题复盘

## 0. 产品方向更新

当前求职版产品聚焦为“基于 RAG 的 Java 代码审查 Agent”。项目理解是审查前的上下文构建能力；新项目生成模块保留已经完成的需求澄清与确认流程，但暂时冻结，不进入当前简历主线。

当前本文只把已有代码、测试和验收可以证明的审查能力写入简历。新项目生成在完成真实生成、编译和测试闭环前，不得写入下面的简历版本。

## 1. 当前可运行的业务闭环

DevMate AI 的产品主流程调整为审查 Java 代码变更：

```text
注册/登录并建立项目权限
  → 导入 Java Git 项目并解析 AST、配置和迁移 SQL
  → 生成指定 revision 的 Git Diff 与覆盖清单
  → PMD 和项目规则分析确定性问题
  → Hybrid RAG 检索调用链、配置、数据库和相关代码
  → Agent 调用受控只读 Tool 收集证据
  → 输出结构化 Finding、未审查范围和工具链路
  → 开发者反馈并使用固定标准答案评测
```

项目理解保留为审查上下文和辅助阅读能力，而不是本轮继续扩张的第二条开发主线。

Vue 保留用于建立代码证据的“一键解析”，并新增用户显式触发的“开始代码审查”。审查入口由 Java 编排器依次执行源码刷新、Diff、静态分析、Embedding 和 Agent 审查；任一步失败即停止，页面展示失败阶段和恢复动作，避免继续消耗模型请求。固定流水线和单步入口仍保留为评测及排障工具。
...
- 一键审查编排、请求幂等、同项目并发互斥、超时回收和可读失败恢复。
...
### 问题十：任务结束后为什么仍提示“正在运行”

一键审查最初使用 MyBatis-Plus `updateById` 把 `running_key` 设为 `null`，但默认更新策略会跳过空值，数据库唯一运行键没有真正释放，导致下一次审查一直冲突。修复后结束、失败和超时路径都使用显式更新把字段置空，并增加重复与并发状态测试。这个问题说明 Java 对象中的 `null` 不等于数据库一定执行 `SET column = NULL`。

## 5. 演示顺序

1. 导入一个包含至少两次提交的 Java Git 项目并完成源码解析。
2. 在理解工作台点击“开始代码审查”，观察五个阶段和失败恢复信息。
3. 打开审查报告，展示 Diff 覆盖、静态 Finding、RAG 证据和 Agent Tool 调用链。
4. 回到源码结构与检索界面，解释具体 Finding 如何对应真实文件、符号和行号。
5. 面试重点解释事务边界、AST/RAG 证据、幂等与并发键，以及失败后为什么不会继续调用模型。
