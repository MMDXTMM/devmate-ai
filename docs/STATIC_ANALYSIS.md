...
静态分析负责模型不应该猜测的问题。当前流水线使用 PMD 7.26.0 和 DevMate 项目规则，对目标版本中本次 Diff 涉及的 Java 文件运行受控检查，并只保留位置与目标变更行相交的问题。
...
校验文件仍位于受控源码工作区，限制文件数和执行时间
        ↓
PMD 使用固定规则集分析源码（限时工作线程）
        ↓
读取已持久化的调用、配置和数据访问关系
        ↓
DevMate 项目规则分析事务、循环访问和锁内IO
        ↓
过滤未命中本次目标变更行的历史问题
        ↓ 短事务
...
| 项目规则 | 分类 | 证据边界 |
|---|---|---|
| `TransactionalSelfInvocation` | `TRANSACTION` | 变更行在同类内部调用唯一解析且带 `@Transactional` 的目标方法 |
| `DataAccessInsideLoop` | `PERFORMANCE` | 变更行的数据访问命名线索位于 AST 循环上下文，提示验证 N+1/请求放大 |
| `BlockingDataAccessUnderLock` | `CONCURRENCY` | 变更行的数据访问命名线索位于同步方法或同步块，提示验证持锁慢 IO |

后两条明确使用“可能风险”和“请验证”，因为 Mapper/Repository 命名只能证明调用形态，不能证明运行时数据库延迟或业务数据量。
...
- 数据库上下文在请求线程读取；只有 PMD 等耗时源码工具进入可取消的限时线程，避免跨线程事务可见性问题。
...
6. 为什么项目规则要区分“确定事实”和“待压测验证的风险”。
7. 为什么不能让限时虚拟线程隐式依赖调用线程尚未提交的数据库事务。
